...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit Owner</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('superadmin.owners.update', $owner) }}">
            ...
            <div class="mb-4">
                ...
                <select name="business_id" id="business_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('business_id') border-red-500 @enderror" required>
                    ...
                    @foreach($businesses as $business)
                        <option value="{{ $business->id }}" {{ old('business_id', $owner->business_id) == $business->id ? 'selected' : '' }}>
                            {{ $business->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $owner->name) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('name') border-red-500 @enderror" required>
                @error('name')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="phone" id="phone" value="{{ old('phone', $owner->phone) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 @error('phone') border-red-500 @enderror" required>
                @error('phone')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
